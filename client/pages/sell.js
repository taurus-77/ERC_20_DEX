...
export default function Home() {
  ...
  async function sellTKNs(event) {
    event.preventDefault();
    setLoading(true);
    let quantity = 0;
    try {
      quantity = event.target[0].value;
      quantity = (Number.parseFloat(quantity) * tokenPrice).toString();
      if (quantity <= 0) {
        setLoading(false);
    ...
    try {
      await TokenContract.methods.approve(contractsAddr.dex, quantity).send({
        from: account
      });

      await DexContract.methods.sell(quantity).send({
        from: account
      });
